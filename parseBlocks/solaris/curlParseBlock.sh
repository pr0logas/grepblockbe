...
function insertBlockNumInTMPfile() {
	medianTime="$(cat $dataFileBlocks | grep mediantime | grep -o '[0-9]*')"
	sed -i "/time/d" $dataFileBlocks
	sed -i "8i\ \ \"mediantime\": $medianTime\," $dataFileBlocks
	sed -i "8i\ \ \"time\": $medianTime\," $dataFileBlocks	
