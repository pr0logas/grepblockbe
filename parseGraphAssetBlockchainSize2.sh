...
if [ -s $formatingFile ]; then
        ...
        echo " \"name\" : \"BlockChainSize\"" >> $formatingFile
        echo $addComma >> $formatingFile
        echo " \"unit\" : \"BlockChainSize\"" >> $formatingFile
        echo $addComma >> $formatingFile
...
sumBlocks=$(tail -n10 $formatingFile | grep y | tail -1 | grep -o '[0-9]*')

# Check if DB works
mongo --host $mongoHost --port $mongoPort --eval "db.blocks.find({\"block\" : 1}).limit(1);" --quiet $database | grep -o -P '."block".{0,16}' | grep -o '[0-9]*' > /dev/null
        ...
        lastProgressBlockInFile=$(($lastProgressBlockInFile + 1))
                ...
                do
               	...
               	start=$(($(date +%s%N)/1000000))
                        ...
                        blockSize=$(mongo --host $mongoHost --port $mongoPort --eval "db.blocks.find({\"block\" : $i}, {size:1, _id:0})" --quiet $database | grep -o '[0-9]*')
                        ...
                        if [[ $formatThisDayRevA -eq $formatNextDayRevA ]]; then
                                ...
                                echo "$setDateStamp Next day found. Total blockchain size: ${sumBlocks} bytes we at $ConvertUnixTime now" 
        			...
        			runtime=$((end-start))
                                ...
                                echo "$setDateStamp Counting blockchain size: ${sumBlocks} bytes. Block: $i/$lastBlockInDB & $ConvertUnixTime. Processing: $runtime ms. Time left approx.: $timeLeftRegardms days"
                        fi
