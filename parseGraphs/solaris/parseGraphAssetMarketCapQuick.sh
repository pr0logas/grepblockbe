...
for (( ; ; ))
	...
	searchingForPrice=$(mongo --host $mongoHost --port $mongoPort --eval "db.priceDataUSD.find({\"unix_time\" : $lastProgressInDB1}, {_id:0}).limit(1)" --quiet $database | grep -o -P '"market_cap".{0,50}' | head -2 | tail -1 | sed 's@,.*@@' | grep -o '[0-9,.-.]*')
