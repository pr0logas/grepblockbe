...
for (( ; ; ))
	...
	if [ -z "$lastProgressInDB1" ]; then
        ...
        searchingForActiveWlt=$(mongo --host $mongoHost --port $mongoPort --eval "db.wallets.find({\"walletTime\" : { $gt : $searchActiveWltMinus3mos}}).count()" --quiet $database | grep -o '[0-9,.-.]*')
                    if [ -z "$searchingForActiveWlt" ]; then
                        searchingForActiveWlt="null"
                    else
                        echo "" > /dev/null
                    fi
                                # Format JSON
