...
for (( ; ; ))
	...
	lastProgress=$(echo "$lastProgress + $averageBlkMinus" | bc) 
        ...
        lastProgressInDB1=$(mongo --host $mongoHost --port $mongoPort --eval "db.wallets.find({\"walletTime\" : { \$gt: $lastProgress}}).sort({\$natural: 1}).limit(1)" --quiet $database | grep -o -P '."walletTime".{0,16}' | head -1 | grep -o '[0-9]*')
