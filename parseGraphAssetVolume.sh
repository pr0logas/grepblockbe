...
for (( ; ; ))
	...
	searchingForPrice=$(mongo --host $mongoHost --port $mongoPort --eval "db.historicalPriceData.find({\"unix_time\" : $lastProgressInDB1}, {_id:0}).limit(1)" --quiet $database  | jq -r '.market_data.total_volume.usd')
