...
for (( ; ; ))
	...
	searchingForPrice=$(mongo --host $mongoHost --port $mongoPort --eval "db.historicalPriceData.find({\"unix_time\" : $lastProgressInDB1}, {_id:0}).limit(1)" --quiet $database | grep -o -P '"usd".{0,50}' | tail -1 | sed 's@,.*@@' | grep -o '[0-9,.-.]*')
