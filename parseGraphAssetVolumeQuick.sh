...
for (( ; ; ))
	...
	lastProgressInDB1=$(mongo --host $mongoHost --port $mongoPort --eval "db.priceDataUSD.find({\"unix_time\" : { \$gt: $lastProgress}}).sort({\$natural : 1}).limit(1)" --quiet $database | grep -o -P '."unix_time".{0,16}' | head -1 | grep -o '[0-9]*')
	# Search for USD price
	searchingForPrice=$(mongo --host $mongoHost --port $mongoPort --eval "db.priceDataUSD.find({\"unix_time\" : $lastProgressInDB1}, {_id:0}).limit(1)" --quiet $database | grep -o -P '"total_volume".{0,50}' | tail -1 | sed 's@,.*@@' | grep -o '[0-9,.-.]*')
	...
	if [[ "$lastProgressInDB1" -eq "$lastProgressInDB2" ]]; then
		...
		# Copy JSON to production
                ...
                dateProgress=$(date -d @${lastProgressInDB1} +'%Y-%m-%d-%T')
                setDateStamp=$(date +%Y-%m-%d\|%H:%M:%S\|%N)
                echo "$setDateStamp Added data to graph: $dateProgress & Volume: \$ ${searchingForPrice}"
	fi
done
