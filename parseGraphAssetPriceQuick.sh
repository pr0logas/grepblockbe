...
for (( ; ; ))
	...
	searchingForPrice=$(mongo --host $mongoHost --port $mongoPort --eval "db.priceDataUSD.find({\"unix_time\" : $lastProgressInDB1}, {_id:0}).limit(1)" --quiet $database | grep -o -P '"current_price".{0,50}' | head -1 | sed 's@,.*@@' | grep -o '[0-9,.-.]*')
	...
	if [[ "$lastProgressInDB1" -eq "$lastProgressInDB2" ]]; then
		...
		setGraphTimeFormat=$(echo "${lastProgressInDB1} * 1000" | bc)
